...
if (isset($_SESSION['email'])) {
    if (in_array('add_new_project', $permission[$user['position']]) !== FALSE) {
    ...
    if (!isset($_POST['add_new_project'])) {
        ...
        if (in_array($action, $permission[$user['position']]) !== FALSE) {
            ...
            echo '<select name="project_name" size="1">
        ...
        $order = isset($_POST['proj_name']) ? 'projects.proj_name' : '';
        $order = isset($_POST['proj_manager']) ? 'proj_manager' : $order;
        $order = isset($_POST['start_day']) ? 'projects.start_day' : $order;
        $order = isset($_POST['deadline']) ? 'projects.deadline' : $order;
